...
let dragOffsetY = 0;
let gameOver = false;
let dragStartPieceClone = null; // Для хранения клона фигуры при перетаскивании
...
function updateTurnIndicator() {
    ...
    if (indicator) {
        ...
        if (gameOver) {
            ...
            indicator.style.background = 'linear-gradient(135deg, #ffd700 0%, #ffb347 100%)';
...
function setPieceHoldEvents() {
    ...
    board.addEventListener('mousedown', function(event) {
        ...
        clone.classList.add('board__piece_dragging');
        clone.style.position = 'fixed';
        clone.style.zIndex = '1000';
        clone.style.pointerEvents = 'none';
        document.body.appendChild(clone);
        
        // Получаем размеры и позицию оригинальной фигуры
        ...
        // Делаем оригинальную фигуру полупрозрачной
        piece.style.opacity = '0.3';
        
        curHeldPiece = piece;
        dragStartPieceClone = clone;
        curHeldPieceStartingPosition = boardPos;
        isDragging = true;
        
        updatePiecePosition(event.clientX, event.clientY, clone);
    });

    document.addEventListener('mousemove', function(event) {
        if (!isDragging || !dragStartPieceClone) return;
        
        event.preventDefault();
        updatePiecePosition(event.clientX, event.clientY, dragStartPieceClone);
    });

    document.addEventListener('mouseup', function(event) {
        if (!isDragging || !curHeldPiece || gameOver) {
            // Очищаем, если что-то пошло не так
            if (dragStartPieceClone) {
                dragStartPieceClone.remove();
                dragStartPieceClone = null;
            }
            if (curHeldPiece) {
                curHeldPiece.style.opacity = '';
            }
            isDragging = false;
            curHeldPiece = null;
            clearHighlights();
        ...
        // Восстанавливаем видимость оригинальной фигуры
        curHeldPiece.style.opacity = '';
        ...
        // Если ход не был сделан, просто убираем клон
        if (!validMove) {
            // Ничего не делаем с оригинальной фигурой, она уже видима
        }
    ...
    function updatePiecePosition(clientX, clientY, clone) {
        if (!clone) return;
...
function movePiece(piece, startingPosition, endingPosition) {
    ...
    if (boardPiece !== '.') {
        if ((boardPiece === boardPiece.toUpperCase() && curPlayer === 'black') ||
            ...
            const startSquare = document.getElementById(`${startingPosition[0] + 1}${startingPosition[1] + 1}`);
            
            // Очищаем целевую клетку
            destinationSquare.innerHTML = '';
            
            // Удаляем фигуру из начальной клетки и добавляем в целевую
            const pieceElement = startSquare.querySelector('.board__piece');
            if (pieceElement) {
